    ...
    <section className="py-16 md:py-24 bg-cream custom-mobile-spacing">
      <div className="container mx-auto">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-lg text-muted-foreground mt-4">बस 4 आसान steps में पाएं complete intimate wellness</p>
        </div>
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-2 gap-2 gap-y-8 md:grid-cols-2 lg:grid-cols-4 md:gap-8">
            {steps.map((step) => (
              <div
                key={step.id}
                onClick={() => setActiveStep(step.id)}
                className={`relative bg-white rounded-3xl p-3 md:p-8 shadow-soft border-2 transition-all duration-500 cursor-pointer ${
                  activeStep === step.id ? "border-primary shadow-glow scale-105 z-10" : "border-border/50 hover:border-primary/30"
                }`}
              >
                <div
                ...
                <div className="text-center">
                  ...
                  <h3 className="md:text-xl text-lg font-bold text-foreground md:mb-3 mb-0">{step.title}</h3>
                  <p className="text-muted-foreground md:text-sm text-xs leading-relaxed mb-2 md:mb-0">{step.description}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex justify-center gap-2 mt-12">
            {steps.map((step) => (
              <button
                key={step.id}
                onClick={() => setActiveStep(step.id)}
                className={`h-3 rounded-full transition-all duration-300 ${activeStep === step.id ? "bg-primary w-8" : "bg-muted hover:bg-primary/50 w-3"}`}
